...
@router.get("/capsule/features")
def get_capsule_feature_snapshot():
    """
    Snapshot for the core capsule capabilities used by this template:
    ...
    try:
        ...
        try:
            derive_res = client.derive(
                path="app/template/capsule-feature-probe",
                context="snapshot",
                length=16,
            )
            kms["enabled"] = True
